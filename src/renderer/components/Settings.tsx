...
import CustomModelUsageSection from './settings/CustomModelUsageSection';
import {
...
type TabType = 'general' | 'appearance' | 'coworkAgentEngine' | 'model' | 'usage' | 'browserWebAccess' | 'coworkMemory' | 'coworkDreaming' | 'shortcuts' | 'im' | 'email' | 'plugins' | 'about';
...
const Settings: React.FC<SettingsProps> = ({
  ...
  const sidebarTabs: { key: TabType; label: string; icon: React.ReactNode }[] = (() => {
    const allTabs = [
      ...
      { key: 'usage' as TabType,          label: i18nService.t('usageTab'),       icon: <ChartBarIcon className="h-5 w-5" /> },
  ...
  const renderTabContent = () => {
    switch(activeTab) {
      ...
      case 'usage':
        return <CustomModelUsageSection />;
